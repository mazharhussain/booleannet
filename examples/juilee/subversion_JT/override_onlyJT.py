...
def override( node, indexer, tokens, param ):
    ...
    if node == 'PIC':
        ...
        step1 = 'PROP = %s' % prop_text
        step2 = '%s = (1-PROP) * %s' % ( newIL41, piece)
        expr = [ step1, step2 ]
        return expr
...
#        expr = '%s = ((%s + %s) * (%s * 0.4 + %s * 0.2 + %s * 2)) * %s' % (newval, concMP, concRP, concAgAb, concC, concBb, concBb)
